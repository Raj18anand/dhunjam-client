...
const AdminDashboard = ({data}) => {
  ...
  const handleSaveClick=async (event)=>{
    ...
    if(saveButtonDisabled==true) {
        ...
        await axios.put(`https://stg.dhunjam.in/account/admin/${data.id}`,body)
        .then((response) => {
  ...
  const getAdminDetails = async (id) => {
    ...
    .then(response => {
        ...
        setRegularAmounts([res.amount.category_7.toString(), res.amount.category_8.toString(), res.amount.category_9.toString(), res.amount.category_10.toString()]);
    ...
    <div className="admin-container">
      ...
      <form className="form-container">
        ...
        <div className="form-item">
          ...
          className={`custom-text-field ${!chargeCustomers? 'disabled':''}`}
          value={customAmount}
        ...
        <div className="form-item">
          ...
          {regularAmounts.map((amount, index) => (
            <input
              ...
              type="number"
              placeholder=""
              className={`regular-text-field ${!chargeCustomers? 'disabled':''}`}
              value={amount}
        ...
        <div className="form-item">
          <button type="submit" className={`save-button ${saveButtonDisabled?'disabled':''}`} disabled={saveButtonDisabled} onClick={handleSaveClick}>Save</button>
        </div>
      </form>
    </div>
  );
